...
const AdminContacts = () => {
  ...
  const handleStatuesUpdate = async (newStatus) => {
    try {
      await axios.put(
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire("삭제실패", "삭제 중 문제가 발생했습니다.", "error");
      }
    }
  };
  ...
  return (
    <div className="p-4 mx-auto max-w-[1400px]">
      ...
      {contacts.length === 0 ? (
        <div className="text-center px-8 bg-white rounded-lg shadow">
      ...
      {/*inset = 모달창을 클릭할 시 주변 백그라운드의 색이 어두워지는 css설정 */}
      {modalOpen && selectedContact && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-wmd">
